...
::utility::string_t odata_json_reader_minimal::get_edit_link_from_context_url(const ::utility::string_t& context_url)
{
...
void odata_json_reader_minimal::set_edit_link_for_entity_value(const std::shared_ptr<odata_entity_value>& entity_value, const ::utility::string_t& expect_type_name, 
									const ::utility::string_t& edit_link, bool is_collection)
{
	...
	entity_value->set_value(PAYLOAD_ANNOTATION_EDITLINK, edit_link + key_string);
	...
	if (expect_type_name != entity_value->get_value_type()->get_name())
	{
		...
		if (entity_value->has_property(PAYLOAD_ANNOTATION_TYPE))
		{
			...
			entity_value->set_value(PAYLOAD_ANNOTATION_EDITLINK, derived_edit_link);
		}
	}
}

std::shared_ptr<odata_payload> odata_json_reader_minimal::deserilize(const web::json::value& content)
{
	...
	for (auto iter = payload_content.as_array().begin(); iter != payload_content.as_array().end(); ++iter)
	{
		...
		else if (return_type->get_type_kind() == edm_type_kind_t::Entity)
		{
			...
			// calculate edit link
			if (entity_value && !entity_value->has_property(PAYLOAD_ANNOTATION_EDITLINK))
			{
...
void odata_json_reader_minimal::handle_extract_navigation_property(web::json::value& value, std::shared_ptr<edm_navigation_type> navigation_type, odata_property_map& result, const ::utility::string_t& name)
{
	if (!navigation_type)
	{
		return ;
...
odata_property_map odata_json_reader_minimal::handle_extract_entity_property(web::json::value& value, std::shared_ptr<edm_entity_type>& entity_type)
{
	...
	{
		auto annotation_type = 	value[PAYLOAD_ANNOTATION_EDITLINK];
		...
		result[PAYLOAD_ANNOTATION_EDITLINK] = 
			std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_EDITLINK), annotation_value);
	}

	// find odata.type and check odata.type to see if it is a derived type
	if (value.has_field(PAYLOAD_ANNOTATION_TYPE))
	{
		auto annotation_type = 	value[PAYLOAD_ANNOTATION_TYPE];
		auto annotation_value = annotation_type.as_string();
		result[PAYLOAD_ANNOTATION_TYPE] = std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_TYPE), annotation_value);
					...
					if (navigation_value->get_value_type()->get_type_kind() == edm_type_kind_t::Collection)
					{
						...
						if (collection_value)
						{
							for (auto collection_iter = collection_value->get_collection_values().cbegin(); collection_iter != collection_value->get_collection_values().cend(); collection_iter++)
							{
								...
								if (element_value && !element_value->has_property(PAYLOAD_ANNOTATION_EDITLINK))
								{
...
odata_property_map odata_json_reader_minimal::handle_extract_complex_property(web::json::value& obj, std::shared_ptr<edm_complex_type>& complex_type)
{
    odata_property_map result;
	...
	// find odata.type and check odata.type to see if it is a derived type
	if (obj.has_field(PAYLOAD_ANNOTATION_TYPE))
	{
		auto annotation_type = 	obj[PAYLOAD_ANNOTATION_TYPE];
		auto annotation_value = annotation_type.as_string();
		result[PAYLOAD_ANNOTATION_TYPE] = std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_TYPE), annotation_value);
		annotation_value = annotation_value.substr(1, annotation_value.length() - 1);
...
std::shared_ptr<odata_collection_value> odata_json_reader_minimal::handle_extract_collection_property(std::shared_ptr<edm_named_type> type, web::json::value& value)
{
    if (!type)
	{
		return nullptr;
...
void odata_json_reader_minimal::handle_extract_entity_annotation(const ::utility::string_t& annotation, const ::utility::string_t& value, odata_property_map& result)
{
	if (annotation == PAYLOAD_ANNOTATION_EDITLINK)
	{
	...
	else if (annotation == PAYLOAD_ANNOTATION_READLINK)
	{
		result[PAYLOAD_ANNOTATION_READLINK] = 
			std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_READLINK), value);

	}
	else if (annotation == PAYLOAD_ANNOTATION_ID)
	{
		result[PAYLOAD_ANNOTATION_ID] = 
			std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_ID), value);
	}
	else if (annotation == PAYLOAD_ANNOTATION_TYPE)
	{
		result[PAYLOAD_ANNOTATION_TYPE] = 
			std::make_shared<odata_primitive_value>(std::make_shared<edm_payload_annotation_type>(PAYLOAD_ANNOTATION_TYPE), value);
	}
}
